...
public class FeatureExtract_DT {
	...
	public double[] getEdgeFeature(Word_Node z1, Word_Node z2, String question,String[] story) {
...
//			System.out.println(z2.getCurrentSentense().content);
		...
		double[] edge_feature = { sentense_distance, word_distance, is_z1_parent_z2, is_z1_sibling_z2, is_z1_child_z2,
				is_z2_parent_z1, is_z2_sibling_z1, is_z2_child_z1, z1_z2_overlap, w_q_overlap, is_z1_parent_z2_lemma,
				is_z1_sibling_z2_lemma, is_z1_child_z2_lemma, is_z2_parent_z1_lemma, is_z2_sibling_z1_lemma,
				is_z2_child_z1_lemma, z1_z2_overlap_lemma, w_q_overlap_lemma };
		return edge_feature;
	}

	public double[] getNodeFeature(Word_Node w, String question, String[] story) {
		...
		double node_similiar = getWord_OverlapFeature(content_preprocess(w_currentsentense),
				content_preprocess(question));
		double node_similiar_lemma = getWord_OverlapFeature(content_preprocess_lemma(w_currentsentense),
				content_preprocess_lemma(question));
		double node_bigram_similiar = getBiGramsFeature(content_preprocess(w_currentsentense),
				content_preprocess(question));
		double node_bigram_similiar_lemma = getBiGramsFeature(content_preprocess_lemma(w_currentsentense),
				content_preprocess_lemma(question));
		double[] nodefeature = { word_match, word_2vec, word_net, node_similiar, node_similiar_lemma,
				node_bigram_similiar, node_bigram_similiar_lemma };
		return nodefeature;
	}
	...
	public double[] getAnswerFeature(Word_Node[] word_nodes, String answer, String question, String[] story) {
		...
		double[] answerfeature = { question_type[0], question_type[1], question_type[2], question_type[3],
	...
	public double getAnswer_Support_Similiar(Word_Node[] word_nodes, String answer) {
		...
		for (int i = 0; i < word_nodes.length; i++) {
			String word_sentense_content = word_nodes[i].getCurrentSentense().getContent();
