...
public class FeatureExtractSingle {
	/***
	 * @author chenruili
	 */
	 ...
	 * @return feature list
	 */
	public HashMap<String, Double> getAllFeature(Sentence sentense, Statement statement, Question question, Ans ans) {
		HashMap<String, Double> featureMap = new HashMap<>();
		String senContent = sentense.content;
		...
		double sentense_similiar_feature_lemma = getSimiliarFeatureBasedOnSentense(
				contentPreprocessPassSWLemma(senContent),
		...
		double sentense_w2v_similiar_feature_lemma = getW2VSimiliarFeatureBasedOnSentense(
				contentPreprocessPassSWLemma(senContent), contentPreprocessPassSWLemma(statement.getContent()));
		featureMap.put("sentense_w2v_similiar_feature_lemma", sentense_w2v_similiar_feature_lemma);

		double dependency_u_v_s_feautre = getDependency_U_V_Syntax_Feature(statement, sentense);
		featureMap.put("dependency_u_v_s_feautre", dependency_u_v_s_feautre);

		double dependency_u_v_feature = getDepeency_U_V_Featurend(statement, sentense);
		...
		double dependency_svo_u_v_feature = getDependency_SVO_U_V_Feature(statement.getWordNode(), sentense);
		featureMap.put("dependency_svo_u_v_feature", dependency_svo_u_v_feature);

		double dependency_so_u_v_s_feature = getDependency_SO_U_V_Syntax_Feature(statement.getWordNode(), sentense);
		featureMap.put("dependency_so_u_v_s_feature", dependency_so_u_v_s_feature);

		double dependency_so_u_v_feature = getDependency_SO_U_V_Feature(statement.getWordNode(), sentense);
		...
		double is_sharing_q_a_word_feature = getIsSharingQAWordFeature(contentPreprocessPassSWLemma(ansContent),
				contentPreprocessPassSWLemma(queContent), contentPreprocessPassSWLemma(senContent));
		...
		if (is_sharing_q_a_word_feature == 1) {
			inner_sentense_distance_feature = getInnerSentenseDistanceFeature(
					contentPreprocessPassSWLemma(senContent), contentPreprocessPassSWLemma(ansContent),
					contentPreprocessPassSWLemma(queContent));
		}
		featureMap.put("inner_sentense_distance_feature", inner_sentense_distance_feature);

		double frame_target_name_feature = getFrameTargetNameFeature(statement, sentense);
		featureMap.put("frame_target_name_feature", frame_target_name_feature);

		double frame_target_text_feature = getFrameTargetTextFeature(statement, sentense);
		featureMap.put("frame_target_text_feature", frame_target_text_feature);

		double frame_target_all_feature = getFrameTargetAllFeature(statement, sentense);
		featureMap.put("frame_target_all_feature", frame_target_all_feature);

		double frame_element_name_feature = getFrameElementNameFeature(statement, sentense);
		featureMap.put("frame_element_name_feature", frame_element_name_feature);

		double frame_element_text_feature = getFrameElementTextFeature(statement, sentense);
		featureMap.put("frame_element_text_feature", frame_element_text_feature);

		double frame_element_all_feature = getFrameElementAllFeature(statement, sentense);
		featureMap.put("frame_element_all_feature", frame_element_all_feature);
		
		double frame_element_target_name_feature = getFrameElementTargetNameFeature(statement, sentense);
		featureMap.put("frame_element_target_name_feature", frame_element_target_name_feature);
		
		double frame_element_target_text_feature = getFrameElementTargetTextFeature(statement, sentense);
		featureMap.put("frame_element_target_text_feature", frame_element_target_text_feature);
		
		double frame_element_target_all_feature = getFrameElementTargetAllFeature(statement, sentense);
	...
	public double getSimiliarFeatureBasedOnSentense(String sentense, String statement) {
		boolean question_neg = get_Negation_Feature(statement);
		boolean sentense_neg = get_Negation_Feature(sentense);
		double score = getWordOverlapFeature(statement, sentense);
		if (!question_neg && sentense_neg) {
			score = score / 2;
		}
		return score;
	}
	...
	public double getDepeency_U_V_Featurend(Statement statement, Sentence sentense) {
		WordNode[] statement_words = statement.getWordNode();
		WordNode[] sentense_words = sentense.wordNodes;
		double score = 0;
		if (statement_words != null && sentense_words != null) {
	...
	public double getIsSharingQAWordFeature(String answer, String question, String sentense) {
		double score = 0;
	...
	public double getInnerSentenseDistanceFeature(String sentense, String answer, String question) {
		double minDis = 1;
		String[] answer_word = answer.split(" ");
		String[] question_word = question.split(" ");
	...
	public boolean get_Negation_Feature(String string) {
		boolean flag = false;
		string = string.toLowerCase();
		String[] negation_words = { "not", "didn", "doesn", "don", "isn", "aren" };
		String[] question_words = string.split(" ");
	...
	public double get_Is_Sharing_VB_Q_A_Word_Feature(String answer,Question que,String sentense){
		double score = 0;
		String[] answer_word = answer.split(" ");
		String[] sentense_word = sentense.split(" ");
		boolean flag1 = false;
